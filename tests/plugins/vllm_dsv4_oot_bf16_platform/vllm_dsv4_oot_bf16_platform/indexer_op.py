...
"""BF16 Sparse Attention Indexer op for the DeepSeek V4 OOT fallback.

Structurally identical to the in-tree FP8 ``dsv4_sparse_attn_indexer`` op,
but reads/writes the BF16 indexer K cache and calls the plugin's BF16
kernels. Registered under the op name ``dsv4_sparse_attn_indexer_bf16`` on
the shared ``vllm_hw_agnostic`` library so ``BF16SparseAttnIndexer`` can
dispatch to it.
"""
...
from vllm_dsv4_oot_bf16_platform.kernels._bf16_layout import kv_cache_uses_fp8
from vllm_dsv4_oot_bf16_platform.kernels.mqa_logits_torch import (
    mqa_logits_torch,
    paged_mqa_logits_torch,
)
from vllm_dsv4_oot_bf16_platform.kernels.indexer_quant_cache import (
...
def dsv4_sparse_attn_indexer_bf16(
    ...
    if not isinstance(attn_metadata, dict):
        ...
        return dsv4_sparse_attn_indexer_bf16_fake(
            hidden_states,
            k_cache_prefix,
            kv_cache,
            q_quant,
...
def dsv4_sparse_attn_indexer_bf16_fake(
    hidden_states: torch.Tensor,
    k_cache_prefix: LayerNameType,
    kv_cache: torch.Tensor,
    q_quant: torch.Tensor,
...
def register_bf16_indexer_op() -> None:
    """Register the BF16 sparse-indexer op once, on the shared library."""
    global _REGISTERED
    if _REGISTERED:
        return
    direct_register_custom_op(
        op_name="dsv4_sparse_attn_indexer_bf16",
        op_func=dsv4_sparse_attn_indexer_bf16,
        mutates_args=["topk_indices_buffer"],
        fake_impl=dsv4_sparse_attn_indexer_bf16_fake,
