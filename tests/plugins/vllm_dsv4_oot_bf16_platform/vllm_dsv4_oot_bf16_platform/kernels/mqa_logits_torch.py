...
"""MQA logits for the sparse indexer, over the FP8 or BF16 packed slot."""
import torch
...
def mqa_logits_torch(
    ...
    """MQA logits for a single sequence without KV paging.

    K is dequantized to bf16 up front, so this is layout-agnostic; the
    caller passes a per-KV-token ``scale`` (1.0 in the bf16 case).

    Args:
        q: Query tensor of shape [M, H, D].
        kv: Tuple `(k, k_scales)` where `k` has shape [N, D] and `k_scales`
            has shape [N] (or [N, 1]), dtype `torch.float32`.
...
def paged_mqa_logits_torch(
    ...
    """Paged MQA logits over the DSv4 packed slot, FP8 or BF16 layout.

    Two paths, selected by ``next_n``:
      * ``next_n == 1``: CUDA-graph-safe fast path (no ``.item()`` syncs, no
        per-batch Python loop); masks invalid positions against
        ``context_lens`` over the full padded shape.
      * ``next_n > 1``: general MTP / speculative-decode per-request block loop.

    The two cache layouts differ only in how the packed slot is decoded:
      * FP8 (``use_fp8``): ``dim`` fp8 value bytes + a 4-byte fp32 UE8M0 scale
        per token; the K values are dequantized by the per-token ``scale``.
      * BF16 fallback: ``dim`` bf16 value bytes, no scale block.
    """
    ...
    if next_n == 1:
        ...
        # FP8 slot = dim value bytes + 4B fp32 scale; BF16 slot = dim*2 bytes.
        head_width = dim + 4 if use_fp8 else dim * 2
        ...
        if cache_scale is not None:
            # NOTE: the FP8 dequant scale is applied AFTER the head-sum, not
            # folded into ``cache_value`` before the einsum. Folding earlier is
            # mathematically equal (UE8M0 scale > 0, relu(s*x)=s*relu(x)) but
            # NOT bit-identical -- distributing the multiply across the sum
            # changes float rounding. Keep it post-sum so the FP8 path stays
            # byte-identical to the pre-consolidation kernel.
    ...
    # next_n > 1 (MTP / speculative decode): per-request block loop.
    q = q.float()
    if use_fp8:
        # NOTE: here the FP8 scale IS folded into the K values before the
        # matmul (this is what the original DeepGEMM reference does). It differs
        # from the ``next_n == 1`` path above, which applies the scale post-sum;
        # both are kept exactly as-is so each path stays byte-identical to its
