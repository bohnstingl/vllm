...
"""Fused compress + RoPE + quant + cache insert kernels (FP8 or BF16)."""
...
def _fused_kv_compress_norm_rope_insert_sparse_attn(
    ...
    """Fused compress → RMSNorm → quant (nope) → RoPE → store.

    One program per token; early-exits for non-boundary positions. FP8
    cache block layout (``block_size`` tokens); the BF16 fallback stores
    all 512 values as bf16 with no scale block:
...
def _fused_kv_compress_norm_rope_insert_indexer_attn(
    ...
    """Fused compress → RMSNorm → RoPE → quant → store.

    One program per token; early-exits for non-boundary positions. FP8
    cache block layout; the BF16 fallback stores 128 bf16 values/token
    with no scale block:
