...
"""Sparse MLA decode: dequantize the paged cache to BF16, then BF16 sparse MLA."""

import torch

from vllm_dsv4_oot_bf16_platform.kernels._bf16_layout import (
    BF16_TOKEN_BYTES,
    FP8_TOKEN_DATA_SIZE,
    MLA_NOPE_DIM,
    MLA_ROPE_DIM,
...
def dequant_gather_slots(
    ...
    """Gather cache pages at scattered slot indices into bf16.

    FP8 UE8M0 pages are dequantized; BF16 pages are copied through.
    """
...
def triton_sparse_decode(
    ...
    if not swa_only and topk_idx_2d is not None and kv_cache is not None:
        ...
        dequant_gather_slots(
            topk_buf, kv_cache, topk_flat, compressed_block_size, use_fp8=use_fp8
        )
    ...
    dequant_gather_slots(
        swa_buf, swa_kv_cache, swa_flat, swa_block_size, use_fp8=use_fp8
    )
