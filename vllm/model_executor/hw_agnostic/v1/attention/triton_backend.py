...
"""Hardware-agnostic Triton attention backend for the hw-agnostic layer path.

A portable copy of ``vllm.v1.attention.backends.triton_attn`` reached through
the Transformers-backend resolver when ``VLLM_USE_HW_AGNOSTIC`` is set. The
metadata, builder, and the unified-attention core are kept verbatim; the
accelerator-specific fast paths of the in-tree backend are dropped to a
portable floor: the ROCm ``rocm_aiter_ops`` fused rope+cache path and the Intel
XPU tensor-descriptor toggle are removed, and the CUDA device-capability KV
dtype gate is not re-imposed here (a hardware vendor supplies those via an
out-of-tree backend). The Triton kernels are imported unchanged from vLLM —
they are already portable and carry no vendor calls."""
...
class TritonAttentionImpl(AttentionImpl):
    ...
    def __init__(
        ...
        # The in-tree backend raises here on CUDA SMs that lack native FP8 /
        # bfloat16 KV cache. That gate is accelerator-specific; the portable
        # floor leaves KV-dtype capability to the platform / OOT backend.
        ...
        # FP8 query quantization is a CUDA fast path; off on the portable floor.
        self.supports_quant_query_input = False

        self._kv_quant_mode = get_kv_quant_mode(kv_cache_dtype)
        self._is_per_token_head_quant = self._kv_quant_mode.is_per_token_head

        # Tensor descriptors (Intel XPU 2D block reads) are an accelerator
        # fast path; the portable floor keeps them off. The Triton kernel
        # eliminates the dead branch at compile time, so this is free.
        self.use_td = False
    ...
    # The in-tree backend overrides `fused_rope_kvcache_supported` /
    # `do_rope_and_kv_cache_update` with the ROCm `rocm_aiter_ops` fused
    # rope+cache kernel. That is vendor-specific; the portable floor inherits
    # the base `AttentionImpl` defaults (fusion unsupported), so RoPE runs as
    # its own op and the KV-cache update goes through `do_kv_cache_update`.
