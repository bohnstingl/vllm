...
class SpyreAttentionPagedMetadataBuilder(AttentionMetadataBuilder[SpyreAttentionPagedMetadata]):
    ...
    def build(
        ...
        # Store a sentinel (True) for causal masking when prefill is happening;
        # the actual mask is computed per-call in the impl since it depends on
        # per-sequence lengths which can vary batch-to-batch.
        causal_mask = None
        if common_attn_metadata.causal and max_query_len > 1:
            causal_mask = torch.tensor(True, device=self.device)
...
class SpyreAttentionPagedImpl(AttentionImpl[SpyreAttentionPagedMetadata]):
    ...
    def forward(
        ...
        # Step 1: Update KV cache (vectorized scatter — no Python loop)
